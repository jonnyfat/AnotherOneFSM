...
TEST(aofsm_StateMachine, instantiate) {
  ...
  const Transition_t transitions[]
  {
      ...
      {                       Event_t::kDefaultEvent,
                              {&SimlpeClient::DefaultAction1, &SimlpeClient::DefaultAction2}
      },
      // Default-Transitions
      {                       Event_t::kDefaultEvent,   State_t::kFinalState,
                             {&SimlpeClient::DefaultTransition1, &SimlpeClient::DefaultTransition2}
      },
      // Transitions
      ...
      {State_t::kBState,      Event_t::kEndEvt,        State_t::kFinalState,
                              {&SimlpeClient::DoPreEndB, &SimlpeClient::DoEndB}
      }
  };
  // clang-format on
